...
# %% [markdown]
FORCING: bool = False
MODEL_START: datetime | None = datetime(2024, 1, 1)
MODEL_END: datetime | None = datetime(2024, 12, 31)
if FORCING:
    ### Inlezen basismodel
    ...
    modflow_budgets_path = resolve_mfms_path(
        settings.source_data_dir.joinpath("GRAM3_2", "100", "GRAM32_BASIS1_TA-PRJ")
    )
...
if MODEL_START or MODEL_END:
    model = Model.read(settings.LHM_WAM_RVW_toml_path)
    model.starttime = MODEL_START or model.starttime
    model.endtime = MODEL_END or model.endtime
    model.write(settings.LHM_WAM_RVW_toml_path)
...
graph, substances = generate(settings.LHM_WAM_RVW_toml_path, settings.LHM_WAM_Delwaq_output_dir)

# Runnen van Delwaq
dimr_config = settings.LHM_WAM_Delwaq_output_dir / "dimr_config.xml"
...
model = parse(
    settings.LHM_WAM_RVW_toml_path, graph, substances, output_folder=settings.LHM_WAM_Delwaq_output_dir, to_input=True
)
...
tracers = (
    ["Initial", "Precipitation", "LevelBoundary", "FlowBoundary"]
