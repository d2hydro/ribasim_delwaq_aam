...
# Zorg dat je VSCode hebt geopent via het `open-vscode.cmd` bestand in de hoofdmap. 
...
# We nemen hier het HSA model als voorbeeld, als je een ander Ribasim model wilt inladen: 
# verander het `toml_path` om een ander Ribasim model in te lezen. 
...
# Het HSA model heeft nog geen concentraties. 
...
# Hier voegen we 2 tracers toe, aan 2 test basins, en geven ze een label. 
# (Dit crasht nog omdat de kolommen er nog niet in zitten(?))
...
# Mocht het crashen op het niet kunnen vinden van Ribasim dan kan je het pad naar Ribasim aanpassen in het `.env` bestand in de hoofd map.


toml_path = settings.processed_data_dir.joinpath("hsa_model") / model.filepath.name
