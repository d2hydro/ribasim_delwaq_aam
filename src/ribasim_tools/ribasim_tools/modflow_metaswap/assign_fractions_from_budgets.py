...
def mfms_budget_to_fraction(budget: str, prefix: str | None = None) -> str:
    """Make fraction string from budget."""
    if "_" in budget:
        bdg_part, sys_part = budget.split("_")
        fraction = bdg_part.removeprefix("bdg") + sys_part.removeprefix("sys")

    else:
        fraction = budget.removeprefix("bdg").removesuffix("m3")
    if prefix is not None:
        fraction = f"{prefix}_{fraction}"
    return fraction
