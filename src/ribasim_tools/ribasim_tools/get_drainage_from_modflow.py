...
def budgets_from_dir(modflow_budgets_path: Path, start_time: pd.TimeStamp, end_time: pd.TimeStamp):
    time_slice = slice(pd.Timestamp(start_time), pd.Timestamp(end_time))
    ar = (
        imod.idf.open(modflow_budgets_path / "modflow" / "bdgriv/bdgriv_sys1_*_l*.IDF")
        .sum(dim="layer")
        .drop_vars(["dy", "dx"])
        .sel(time=time_slice)
    )
    ...
    for isys in range(2, 7):
        ...
        ds[f"bdgriv_sys{isys}"] = (
            imod.idf.open(modflow_budgets_path / "modflow" / f"bdgriv/bdgriv_sys{isys}_*_l*.IDF")
            .sum(dim="layer")
            .drop_vars(["dy", "dx"])
            .sel(time=time_slice)
        )
    ...
    for isys in range(1, 4):
        ...
        ds[f"bdgdrn_sys{isys}"] = (
            imod.idf.open(modflow_budgets_path / "modflow" / f"bdgdrn/bdgdrn_sys{isys}_*_l*.IDF")
            .sum(dim="layer")
            .drop_vars(["dy", "dx"])
            .sel(time=time_slice)
        )
    ...
    bdgsw = (
        imod.idf.open(modflow_budgets_path / "metaswap" / "bdgPssw/bdgPssw_*_l*.IDF")
        .sum(dim="layer")
        .drop_vars(["dy", "dx"])
    )
    bdgqr = (
        imod.idf.open(modflow_budgets_path / "metaswap" / "bdgQrun/bdgQrun_*_l*.IDF")
        .sum(dim="layer")
        .drop_vars(["dy", "dx"])
    )
...
class AssignOfflineBudgets:
    def __init__(
        self,
        modeflow_budgets_path: Path | str = "Basisgegevens/LHM/4.3/results/LHM_433_budget.zip",
    ):
        self.cloud: _CloudStorage | None = None
        if CloudStorage is not None:
            self.cloud = CloudStorage()
            self.modflow_budgets_path = self.cloud.joinpath(modeflow_budgets_path)
        else:
            self.modflow_budgets_path = Path(modeflow_budgets_path)
    ...
    def _sync_files(
        ...
        filepaths = [self.modflow_budgets_path]
        ...
        if self.modflow_budgets_path.is_dir():
            ...
            raise ValueError(
                f"{self.modflow_budgets_path} does not seem to be a zarr store or modflow budgets directory so can't be opened"
            )
